    ...
    <header className="sticky top-0 z-50 bg-white border-b" style={{ borderColor: "var(--color-ink-100)" }}>
      <div className="max-w-6xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-8 shrink-0">
          {navLinks.map((link) => (
            <NavLink key={link.to} to={link.to} className="text-sm font-medium transition-colors duration-150"
              style={({ isActive }) => ({ color: isActive ? "var(--color-leaf-700)" : "var(--color-ink-500)" })}>
              {link.label}
            </NavLink>
          ))}
        </nav>

        <a href="#contact" className="hidden lg:inline-flex items-center rounded-lg text-white text-sm font-semibold px-5 py-2 transition-colors duration-150 shrink-0"
        ...
        <button className="lg:hidden p-2 rounded shrink-0" onClick={() => setOpen(!open)}
          style={{ color: "var(--color-ink-700)" }} aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open}>
          {open
      ...
      {open && (
        <nav className="lg:hidden bg-white border-t px-4 py-4 flex flex-col gap-4" style={{ borderColor: "var(--color-ink-100)" }}>
          {navLinks.map((link) => (
            <NavLink key={link.to} to={link.to} onClick={() => setOpen(false)} className="text-base font-medium"
